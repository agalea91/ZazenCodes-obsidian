...
Common pattern for gs (cloud buckets) is to append the project ID to the start of the bucket name, to guarantee global uniqueness
...
Top rows for each group
(equivalent of groupby(col).head(1) operation in pandas)
```sql
select
    airport,
    array_agg(
...
Vertex AI = fully managed Jupyter notebooks
...
- Discretize dataset along select features
e.g.
...
Deploy endpoint. Note that ENDPOINT_NAME will not change even though above MODEL_NAME will change as different models are deployed
...
Batching is more efficient (cheaper, faster). Beam will tune the batch sizes with this code:
